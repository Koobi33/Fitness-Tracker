...
//Get the most probably class from all data
function mostProbablyClass(data) {
  const mostClass = data.find((item) => item.probability > 0.75 && item.probability < 1);
  return mostClass || {};
}

//This function updates data about our exercise
export function updateMovesSteps(data) {

  data.forEach(({ className }) => {

    const classExist = counters.some((item) => item[0] === className);

    if (!classExist) {
      counters.push([className, false, 0]);
    }
  });

  const { className, probability } = mostProbablyClass(data);

  if (className && counters.length > 0) {
    counters.forEach((item) => {
      if (item && item[0] === className) {
        item[1] = true;
        if (probability > item[2]) {
          item[2] = probability;
        }
      }
    })
  ...
  //Check how your moves similar to example
  function checkActionQuality() {
    let result;
    if (counters.every((item) => item[2] >= 0.95)) {
      result = 5;
    } else if (counters.every((item) => item[2] >= 0.90)) {
      result = 4;
    } else if (counters.every((item) => item[2] >= 0.85)) {
      result = 3;
    } else if (counters.every((item) => item[2] >= 0.80)) {
      result = 2;
    } else {
      result = 1;
    }
  ...
  //Increase counter
  const everyCounterTruthy = counters.every((item) => item[1] === true);
  if (everyCounterTruthy && semiAction) {
    actionCount += 1;
    let actionCountElem = document.getElementById('action-counter');
    let actionQualityElem = document.getElementById('action-quality');
    actionCountElem.innerHTML = actionCount;
    actionQualityElem.innerHTML = checkActionQuality();
    semiAction = false;
    refreshQuality()
  }
  else if (everyCounterTruthy) {
    semiAction = true;
  }
  if (everyCounterTruthy) {
    refreshCounters();
  }
}
